...
function findContextualCode(value: string): string | undefined {
    const label = '(?:Proton(?: Mail)?\\s+verification\\s+code|verification\\s+code|security\\s+code|验证码|驗證碼|code)';
...
export function extractProtonVerificationCode(subject: string, text: string, html: string): string | undefined {
    if (!/proton/i.test(`${subject}\n${text}\n${htmlToVisibleText(html)}`)) return undefined;
...
async function findVerificationInMailbox(
    ...
    try {
        ...
        for (const uid of ids.slice(-10).reverse()) {
            ...
            if (!/proton/i.test(`${subject} ${sender}`))
                continue;

            const verification = extractProtonVerificationCode(
                subject,
                parsed.text ?? '',
                typeof parsed.html === 'string' ? parsed.html : ''
            );
            if (verification && !excludedCodes.has(verification))
                return verification;
        }
    }
    finally {
    ...
    try {
        ...
        while (Date.now() < deadline) {
            for (const mailbox of mailboxNames) {
                const verification = await findVerificationInMailbox(client, mailbox, options.receivedAfter, excludedCodes);
                if (verification)
                    return verification;
            }
            ...
            if (remainingMs <= 0)
                break;
            logger.info('尚未收到 Proton 验证邮件，等待下一次轮询（剩余约 %d 秒）', Math.ceil(remainingMs / 1000));
            await Utility.waitForSeconds(Math.min(pollIntervalMs, remainingMs) / 1000);
        }

        throw new Error(`等待 Proton 验证邮件超时（${Math.round(timeoutMs / 1000)} 秒）`);
    }
    finally {
        await client.logout().catch(() => undefined);
    }
